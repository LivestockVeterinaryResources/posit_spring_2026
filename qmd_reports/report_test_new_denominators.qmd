...
```{r}
get_deno_files<-tibble(file_names = list.files(file.path('data/intermediate_files'))%>%
  filter(str_detect(file_names, 'denominator_by_lact_group'))
                         
herd_deno_new = read_parquet(file.path('data/intermediate_files/denominator_by_lact_group_time_period365.parquet'))%>%
...
```{r}
#monthly -------------------------------------
herd_deno_monthly_new = read_parquet(file.path('data/intermediate_files/denominator_by_lact_group_time_period30.parquet'))%>%
  filter(date_time_period_start>(today()-1200))%>%
  filter(!(deno_type %in% c("dop_lact_basic", "dop_lact_2+", "dop_lact_3+", "dop_lact_5+",'lact_2+')))
...
```{r}
#dim cats-----------------------------------
herd_deno_dim_new = read_parquet(file.path('data/intermediate_files/denominator_by_lact_group_time_period365.parquet'))%>%
  filter(date_time_period_start>(today()-1200))%>%
...
```{r}
herd_deno<-read_parquet(file.path('data/intermediate_files/herd_denominators.parquet'))%>%
...
herd_deno_dim<-read_parquet(file.path('data/intermediate_files/herd_dim_denominators.parquet'))%>%
  mutate(date = lubridate::ymd(paste0(time_period, '-01-01')))%>%
...
### plot monthly
```{r}
herd_deno_monthly<-read_parquet(file.path('data/intermediate_files/herd_season_denominators.parquet'))%>%
