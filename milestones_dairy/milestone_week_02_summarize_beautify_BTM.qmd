...
readr::read_csv().read_csv()
#add code to read in the events file here
events_formatted<-read_csv(path="C:\GitHub\posit_spring_2026-1\data\intermediate_files\events_formatted.parquet")
events_formatted |>
  add_new_variables()
...
#
events_formatted |>
  filter(floordate_month>=2023-04-01, event=="BRED1") |>
  group_by(location_event, lact_group, floordate_month) |>
    dim_at_bred<-summarize(
        mean(dim_event,na.rm=TRUE),
        sd(dim_event,na.rm=TRUE),
        median(dim_event,na.rm=TRUE),
        min(dim_event,na.rm=TRUE),
        max(dim_event,na.rm=TRUE),
        quantile(dim_event, probs = 0.97),na.rm=TRUE
        )
