...
events_formatted <- read_parquet("C:/01_GitHub/01_TidyverseCourse/posit_spring_2026/data/intermediate_files/events_formatted.parquet") |>
  add_new_variables()
...
check<-tibble(list_columns = colnames(events_formatted))%>%
    filter(list_columns %in% c('location_event', 'lact_group', 'event_month'))%>%
    pull(list_columns)%>%
    sort()
...
#Understanding parity variables
lapply(events_formatted[, c("lact_group_basic", "lact_group", "lact_group_repro", "lact_group_5")], unique)
...
#Creating a dataset with only the first breeding for each id_animal_lact
df_bred1 <- events_formatted |>
  filter(event == "BRED1",
         date_event >= max(events_formatted$date_event, na.rm = TRUE) - years(3)) |>
  group_by(id_animal_lact) |>
  ungroup()

#First breeding summary
sm_bred1 <- df_bred1 |>
  group_by(location_event, lact_group, floordate_month = floor_date(date_event, "month")) |>
  summarise(
    ...
    q97    = quantile(dim_event, 0.97, na.rm = TRUE),
    n      = n(),
    .groups = "drop"
  ) |>
  arrange(location_event, lact_group, floordate_month)
sm_bred1
```
...
#| label: compare
#| eval: false
waldo::compare(sm_bred1, solution, tolerance = 1e-4)
```
...
y_limits <- data.frame(
  lact_group = c("Heifer", "LACT 1", "LACT 2", "LACT 3+"),
  q97 = c(800, 400, 300, 250),
  floordate_month = as.Date(NA)
)

ggplot(data = sm_bred1, mapping = aes(x = floordate_month, y = q97)) +
  geom_point(data = y_limits, mapping = aes(y = q97), alpha = 0) +
  geom_point(data = sm_bred1, mapping = aes(color = lact_group)) +
  geom_smooth(data = sm_bred1, mapping = aes(color = lact_group)) +
