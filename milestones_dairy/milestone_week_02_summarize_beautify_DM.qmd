...
ggplot(dim_at_bred, mapping = aes(x=floordate_month, y=q97))+
  geom_point(mapping = aes(color = lact_group),size=0.2)+
  geom_smooth(aes(color= lact_group,fill = lact_group),span = 0.2)+
...
Using the code chunk below, investigate a research question about this data, using the data wrangling skills you learned this week. Bonus: Try something with missing values.

```{r}
dim_at_sold_pre<-events_formatted|>
  filter(
    event=="SOLD1",
    date_event >= (today() - years(3))
    )|>
  mutate(floordate_fresh = floor_date(date_fresh,"months"))|>
  filter(!(is.na(floordate_fresh)))
  
  dim_at_sold<-dim_at_sold_pre|>
    group_by(
    event,
    location_event,
    lact_group,
...
```
```{r}
ggplot(dim_at_sold_pre)+
  #geom_density(aes(x=dim_event))+
  geom_histogram(aes(x=dim_event))+
    facet_wrap(facets=vars(lact_group,location_event))
...
```{r}
ggplot(dim_at_sold, mapping = aes(x=floordate_fresh, y=q97))+
  geom_point(mapping = aes(color = lact_group),size=0.2)+
  geom_smooth(aes(color= lact_group,fill = lact_group),span = 0.2)+
  facet_wrap(facets=vars(lact_group,location_event), scales = "free")+
  labs(title = "DIM Sold by Fresh Date and lactation ",
       x="Fresh Date",
       y="DIM",
       caption="Posit Sample Herd")
