...
events_formatted <- arrow::read_parquet(
...
```{r}
research_missing_data <- events_formatted %>%
  filter(
    event == "BRED1",
    date_event >= max(date_event) - 1095
  ) %>%
  ...
  summarize(
    total_records = n(),
    missing_dim   = sum(is.na(dim_event)),
    total_na_anywhere = sum(rowSums(is.na(across(everything()))) > 0),
    .groups = "drop"
  ) %>%
  ungroup()
