...
#creating object dim_at_bred

dim_at_bred <- events_formatted |>
  filter(
    event == "BRED1",
    date_event >= max(date_event, na.rm = TRUE) - 3*365
  ) |>
  group_by(event, lact_group, location_event, floordate_month) |> ###need to use event in groupby to have BRED1 in 1st column.
  summarize(
