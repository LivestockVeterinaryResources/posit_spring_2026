...
events_formatted<-read_parquet("c:/GitHub/posit_spring_2026/data/intermediate_files/events_formatted.parquet")
events_formatted<-events_formatted|>
  add_new_variables()
check<-events_formatted |>
  select(floordate_month, lact_group, location_event)|>
  names(  )
check
```
...
dim_at_bred<- events_formatted|>
  filter(event=="BRED1")|>
  filter(date_event > max(date_event, na.rm=TRUE) - years(3)) |>
...
##Second attempt - first attempt did not have correct y-axis scales
dim_at_bred|>
  ggplot(mapping=aes(x=floordate_month, y=q97, color=lact_group, fill=lact_group))+
  geom_point()
  geom_line()+
  geom_smooth()+
  facet_wrap(vars(lact_group), scales="free_y")+
  labs(
    x="floordate_month",
    y="q97",
    color="lact_group"
...
##Third attempt - second attempt was very jagged
dim_at_bred|>
  ggplot(mapping=aes(x=floordate_month, y=q97, color=lact_group, fill=lact_group))+
  geom_point()+
  geom_smooth(se=TRUE, span=0.6)+
  facet_wrap(vars(lact_group), scales="free_y")+
  labs(
    x="floordate_month",
    y="q97",
...
##Fourth attempt - the lines are still a bit off & not incorporating some of the points - GOT IT
dim_at_bred|>
  ggplot(mapping=aes(x=floordate_month, y=q97, color=lact_group, fill=lact_group))+
  geom_point()+
  geom_smooth(se=TRUE, span=0.2, linewidth=2.0)+
