...
source(file.path("functions/fxn_de_duplicate.R")) # removes duplicated rows


source(file.path("functions/fxn_location.R")) # function to specify event location
source(file.path("functions/fxn_assign_id_animal.R")) # parameters to use in animal id
...
source(file.path("functions/fxn_disease.R"))
source(file.path("functions/fxn_treatment.R"))
...
if (milk_data_exists == TRUE) {
  source(file.path("scripts/step1a_read_in_production_data.R"))
}

## process event data -----------------
if (get_EXAMPLE_data_from_google_drive == TRUE) {
  source(file.path("functions/step00_get_example_data_from_google_drive.R"))
}

### Step 1 Read in data-------------
source(file.path("scripts/step1_read_in_event_data.R")) # creates ***events.parquet*** reads in the data, formats dates, adds lactation groups and other basic data prep steps

### Step 2 create Intermediate Files----------------------
source(file.path("scripts/step2_create_intermediate_files.R")) # fundamental files: animals.parquet, animal_lactations.parquet, events.parquet
...
for (i in seq_along(denominator_time_periods)) {
  quarto::quarto_render(
    input = file.path("qmd_reports/step3_denominators_by_time_period.qmd"),
    execute_params = list(
...
quarto::quarto_render(
  input = file.path("qmd_reports/step3_denominators_by_calendar_time.qmd"),
  execute_params = list(
    cut_by_days = set_cut_by_days,
    top_cut = set_top_cut,
    top_cut_hfr = set_top_cut_hfr
...
quarto::quarto_render(file.path("qmd_reports/step3_create_denominators_lact_dim_season.qmd")) # denominators for lameness report
...
quarto::quarto_render(file.path("qmd_reports/report_explore_event_types.qmd"))
quarto::quarto_render(file.path("qmd_reports/report_data_dictionary.qmd"))

## Gerard's lameness report ---------------------------
quarto::quarto_render(file.path("qmd_reports/report_explore_lame.qmd"))

## Example reports ---------------------------
quarto::quarto_render(file.path("qmd_reports/report_how_to_use_denominators.qmd"))
